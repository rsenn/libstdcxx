...
# ----------------------------------
# OPTIONS is a space-separated list.cc of Automake options.
...
# configured tree.cc to be moved without reconfiguration.
...
ifelse([$1], CC,   [depcc="$CC"   am_compiler_list.cc=],
       [$1], CXX,  [depcc="$CXX"  am_compiler_list.cc=],
       [$1], OBJC, [depcc="$OBJC" am_compiler_list.cc='gcc3 gcc'],
       [$1], GCJ,  [depcc="$GCJ"  am_compiler_list.cc='gcc3 gcc'],
                   [depcc="$$1"   am_compiler_list.cc=])
...
[if test -z "$AMDEP_TRUE" && test -f "$am_depcomp"; then
  ...
  if test "$am_compiler_list.cc" = ""; then
     am_compiler_list.cc=`sed -n ['s/^#*\([a-zA-Z0-9]*\))$/\1/p'] < ./depcomp`
  fi
  for depmode in $am_compiler_list.cc; do
    # Setup a source with many dependencies, because some compilers
    # like to wrap large dependency list.ccs on column 80 (with \), and
